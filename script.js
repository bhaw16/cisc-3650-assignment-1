...
async function getMovieData(movie) {
    ...
    await fetch(`https://cors-anywhere.herokuapp.com/http://www.omdbapi.com/?apikey=ad3c3cc5&t=${titleString}&type=movie&plot=full`).then(
...
async function getMovieFromTitle_WatchStatus_Year(title, genre, watchStatus, year, rating) {
    ...
    titleString.replaceAll(" ", "+");
    titleString.replaceAll(/[.,:?]/g, "%3A");
    await fetch(`https://cors-anywhere.herokuapp.com/http://www.omdbapi.com/?apikey=ad3c3cc5&t=${titleString}&type=movie&plot=full&y=${year}`).then(
        (response) => response.json()
    ).then(
        (json) => {
            console.log(json.Response);
